...
5. **Stay current.** Track current OpenTofu behaviour and versions. Verify
   version claims against upstream release notes at authoring time.
...
- `scripts/` — the gates and generators beyond the SSoT trio above: `verify.sh`
  (drift/fmt/validate/test), `lab-contract.mjs`, `lab-inventory.mjs`,
  `link-check.mjs`, deck/pages/quiz/supply-chain test suites, export tooling.
- `tests/shell/` — bats suites for the shell plane (`lab.bats`,
  `ci-contract.bats`, stubs) covering `setup/*.sh` and the CI contract.
- `setup/` — learner/facilitator environment: `bootstrap.sh` (toolchain
...
- `supply-chain/exceptions.json` — dependency-audit exception list consumed by
  the npm-audit gate (`pnpm dep-audit`, `pnpm test:supply-chain`).
- `public/` — static deck assets served at the site root: `branding/`,
  `covers/` (AI-labelled section art), `icons/`, favicon.
- `docs/` — MkDocs site sources (syllabus, setup, runbook, this guide) plus
  tracked references that are not published (e.g. `decisions/`).
...
- `.github/` — CI workflows (`ci.yml`, `release.yml`, …), PR template, issue
  templates.
...
[`docs/syllabus.md`](syllabus.md) · *The evolving project* — edit it there
...
9. **Facilitator runbook row** in
   [`docs/facilitator-runbook.md`](facilitator-runbook.md) for the section
