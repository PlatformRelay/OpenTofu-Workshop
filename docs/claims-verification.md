...
Evidence artifact for **US-C-FACTS** — phase 1 gathered the evidence, phase 2
applied the corrections. Every version, licensing, date
...
**Status — the 31 corrections in section L were applied on 2026-08-25**
(US-C-FACTS phase 2). Read the rest of this file with that in mind:

- **Sections A–I record what the repo said _before_ that change.** They are
  the evidence; rewriting them to match the corrected prose would destroy the
  only record of what was wrong. Where a row quotes text that has since been
  corrected, its L-row says so.
- **Section L now quotes what each location reads _today_**, with the
  pre-correction wording beside it. `scripts/claims-check.mjs` therefore
  changed meaning without changing output: it used to prove the pointers had
  not rotted, and now proves the corrections have not been reverted.
- **The two enforcer bumps that the out-of-scope list couples to L20–L26 were
  deliberately NOT applied.** The docs advertise 1.9; `setup/bootstrap.sh` and
  `scripts/verify.sh` still accept 1.8. That is a known, recorded gap — see
  "Out-of-scope findings" at the end.
...
| --- | --- | --- | --- | --- |
| A1 | Current baseline is OpenTofu **1.12.x** | `pages/S10-opentofu-differentiators/index.md:65`; `pages/S01-iac/index.md:456` | VERIFIED | GitHub API `repos/opentofu/opentofu/releases` — latest stable tag `v1.12.6`, published `2026-08-19T11:40:07Z`. |
| A2 | 1.12.x is "supported to 2027-02-01" | `pages/S10-opentofu-differentiators/index.md:65`; `pages/S01-iac/index.md:456` | VERIFIED | `CHANGELOG.md` at tag `v1.12.0`, first line: "The v1.12.x release series is supported until **February 1 2027**." |
| A3 | **1.7** = client-side state **& plan** encryption · provider-defined functions · `removed` block | `pages/S10-.../index.md:55`; `pages/S01-iac/index.md:402-406, 447` | VERIFIED | `CHANGELOG.md` @ `v1.7.0`: "Add support for a `removed` block…"; "Provider-defined functions are now available." For encryption the changelog header reads "STATE ENCRYPTION — We're introducing optional end-to-end encryption for **state files**" and never says *plan*, which initially looked like the deck overclaiming. It is not. The **v1.7-versioned** docs settle it: `website/docs/language/state/encryption.mdx` at tag `v1.7.0` is titled "# State and Plan Encryption" and opens "OpenTofu supports encrypting **state and plan files** at rest, both for local storage and when using a backend." The changelog header is simply loose. (Method note: the *current* docs page carries the same sentence but is versioned v1.12.x, so it cannot adjudicate a 1.7 claim — the tagged docs source can, and does.) |
| A4 | **1.8** = early variable/backend evaluation · `.tofu` extension · test mocking & overrides | `pages/S10-.../index.md:56`; `pages/S01-iac/index.md:448` | VERIFIED | `CHANGELOG.md` @ `v1.8.0` NEW FEATURES: "Variables and Locals allowed in module sources and backend configurations (with limitations)"; "Added support to new .tofu extensions"; "Added support for `override_resource`, `override_data` and `override_module` blocks"; "Added support for `mock_provider`, `mock_resource` and `mock_data` blocks". Release published `2024-07-29T13:04:14Z`. |
| A5 | **1.9** = provider `for_each` · `-exclude` · cross-referencing variable validation | `pages/S10-.../index.md:57`; `pages/S01-iac/index.md:407-416, 449` | VERIFIED | `CHANGELOG.md` @ `v1.9.0`: "**`for_each` in provider configuration blocks**"; "**`-exclude` planning option**"; "References to vars, data, etc. are now usable in variable validation". Published `2025-01-09T16:20:29Z`. |
| A6 | **1.10** = OCI registry for providers *and* modules · external key providers · native S3 state locking | `pages/S10-.../index.md:58`; `pages/S01-iac/index.md:450` | VERIFIED | `CHANGELOG.md` @ `v1.10.0`: "install **module packages from OCI Registries**"; "**OCI Registries as a new kind of provider mirror**"; "**The `s3` backend can now implement locking without DynamoDB**"; "State encryption now supports using external programs as key providers." Published `2025-06-24T13:58:50Z`. |
| A7 | **1.11** = ephemeral resources & write-only attributes · `enabled` meta-arg | `pages/S10-.../index.md:59`; `pages/S01-iac/index.md:451` | VERIFIED | `CHANGELOG.md` @ `v1.11.0`: "**Ephemeral values**… ephemeral resource types… managed resource types with write-only attributes"; "The new **`enabled` meta-argument**". Published `2025-12-09T18:52:00Z`. |
| A8 | **1.12** = dynamic `prevent_destroy` · `destroy = false` · concurrent provider install | `pages/S10-.../index.md:60`; `pages/S01-iac/index.md:451` | VERIFIED | `CHANGELOG.md` @ `v1.12.0`: "A `prevent_destroy` argument… can now refer to other symbols in the same module"; "New `lifecycle` meta-argument `destroy`: when set to `false`…"; "Provider installation now makes concurrent requests to download provider packages". |
| A9 | The 1.7→1.12 span is a genuine divergence window — "several **with no Terraform equivalent**"; provider `for_each` is "**OpenTofu-only** since 1.9" | `pages/S10-.../index.md:39-40, 147, 291, 303`; `pages/S01-iac/index.md:380-384, 421-422, 721` | VERIFIED | Two halves, checked separately. (a) Every feature listed in A3–A8 appears in OpenTofu's own changelog at the version claimed. (b) The *exclusivity* half was checked against **HashiCorp's** docs, not inferred: provider `for_each` — `https://developer.hashicorp.com/terraform/language/providers/configuration` documents only aliasing ("include multiple `provider` blocks with the same provider name, then add the `alias` argument…") and never mentions `for_each` on a provider block; `-exclude` — `https://developer.hashicorp.com/terraform/cli/commands/plan` documents `-target=ADDRESS` as its only resource-targeting option, with no `-exclude`; client-side state encryption — `https://developer.hashicorp.com/terraform/language/state/sensitive-data` states "You can encrypt your state at rest, but the encryption method depends on your specific backend", i.e. delegated to S3/GCS/HCP rather than a native `encryption` block with key providers. All three OpenTofu features named as differentiators are genuinely absent from Terraform. |
...
| C2 | `mock_provider` / `mock_resource` / `mock_data` are **OpenTofu 1.8** | `pages/S17-mocking/index.md:38, 43, 49, 161`; `pages/S01-iac/index.md:448` | VERIFIED | OpenTofu `CHANGELOG.md` @ `v1.8.0`: "Added support for `mock_provider`, `mock_resource` and `mock_data` blocks in testing framework. ([#1772](https://github.com/opentofu/opentofu/pull/1772))". Published `2024-07-29T13:04:14Z`. |
| C3 | `override_resource` / `override_data` / `override_module` are **OpenTofu 1.8** | `pages/S17-mocking/index.md:51`; `pages/S01-iac/index.md:448` | VERIFIED | Same changelog: "Added support for `override_resource`, `override_data` and `override_module` blocks in testing framework. ([#1499](https://github.com/opentofu/opentofu/pull/1499))". |
...
| # | Claim | Where | Verdict | Evidence (primary source, checked 2026-08-25) |
| --- | --- | --- | --- | --- |
| D1 | `TOFU_VERSION=1.10.3` | `versions.env:17`; `pages/S01-iac/index.md:457` | VERIFIED (pin exists) / stale | `repos/opentofu/opentofu/releases` — `v1.10.3` published `2025-07-15T14:33:31Z`. It is not the newest 1.10 patch (`v1.10.10`, `2026-05-11`), and 1.10.x predates the 1.11.x series whose stated support ended 2026-08-01 (A10). The pin is a deliberate, reproducible choice, not an error — but it is two series behind what `pages/S10:65` calls "current baseline". |
| D2 | `GO_VERSION=1.23.6` | `versions.env:27` | VERIFIED (pin exists) / **EOL** | `repos/golang/go` git ref `refs/tags/go1.23.6` exists. `https://go.dev/doc/devel/release`: "Each major Go release is supported until there are two newer major releases", latest major listed **Go 1.27.0 (2026-08-19)** → supported series are 1.26 and 1.27. **Go 1.23 receives no security fixes.** Also newer within its own series: `go1.23.12`. **Superseded 2026-09-04 (Go pin bump):** `GO_VERSION` is now `1.27.1` (`versions.env:27`) with `setup/terratest/Dockerfile`, `setup/bootstrap.sh` `MIN_GO` and the bootstrap self-test moved with it, and `verify.sh` §10 now scans every `labs/**/go.mod` `go` directive against the pin — this row's Claim and Verdict describe the pre-bump tree (where the pin sat at `versions.env:21`); the Where column above points at the pin's current home, per this table's re-derive-by-grepping convention. |
| D3 | `LOCALSTACK_VERSION=4.9.2` | `versions.env:32` | VERIFIED (pin exists) | `repos/localstack/localstack/releases/tags/v4.9.2` — published `2025-10-06T09:01:27Z`. Latest is `v4.14.0` (`2026-02-26`). The comment's rationale ("last community release that boots without `LOCALSTACK_AUTH_TOKEN`") was **not** re-verified in this pass — see U2. |
| D4 | `TERRAMATE_VERSION=0.17.1` | `versions.env:36` | VERIFIED (pin exists) | `repos/terramate-io/terramate/releases/tags/v0.17.1` — published `2026-05-26T13:24:47Z`. Latest is `v0.17.2` (`2026-07-31`). One patch behind; no correction needed. |
...
- **D1 vs A1 — arguable.** `pages/S10:65` calls 1.12.x the "current baseline"
  while `versions.env` pins `TOFU_VERSION=1.10.3` and `pages/S28:69,80,268`
...
## L. Correction list — applied 2026-08-25 (phase 2)

All 31 landed. Each row gives file, line, the text that is **now** there, the
text it replaced, and the row above that carries the citation. One row (L18)
was applied in different markup than phase 1 proposed, and says why in its own
cell rather than anywhere a reader has to go looking.

**Completeness by construction, and the boundary is the REPO — not `pages/**`.**
Phase 2 executed this list literally and blind, so a claim that appears in six
places with only one row would have shipped five-sixths of the defect. Two
things are asserted:
...
2. Every contested claim *family* was then re-grepped across **every tracked
   ...
   The one *tree* bound that applies: `.terraform/` provider caches and
   `node_modules/` are excluded as untracked vendor trees.

   **A second bound is unavoidable and must be stated: phrasing.** A text sweep
   can only find the wordings it thinks to look for, and this repo states the
   same floor at least four ways — `≥ 1.8`, `≥1.8`, `≥ **1.8**`, and
   "1.8 or newer". No single pattern catches all four; the last one contains no
   `≥` and defeats every `≥`-based search. Both times this sweep failed
   (rounds 3 and 5) it failed on the *pattern* axis, not the tree axis. Where a
   claim family is small enough to enumerate — as the per-lab floors are — the
   honest method is to read every candidate file rather than trust a regex, and
   that is what the {06, 10} exception set rests on.
...
requiring `>= 1.9.0`, which made that line false by this document's own
evidence — and `docs/validation-matrix.md:78`, which is Lab 10's *own* row,
and claimed ≥1.8 while the lab it indexes demands 1.9. Both now read ≥1.9.

**Sweep technique, learned by missing things — and the warrant is weaker than
it looks.** Grep the *claim*, not a formatting of it.
`docs/validation-matrix.md:58` reads `≥ **1.8**` with bold markers inside the
number, so a `≥ *1\.8` pattern skips it. `labs/day-1/06-variables.md:63` was
missed because the sweep hunted the *wrong* floor (`1.8`) and never asked which
files assert a *different* one. The fix for that second failure is to enumerate
the exception set directly rather than hunt the majority value:
``grep -rn '`tofu` ≥ [0-9]' labs/``.
...
still holds — the exception set really was {06, 10}, and Labs 16 and 17 really
did need only 1.8 — but it rests on per-file enumeration, not on that grep.
(Past tense since 2026-08-31: US-D-VERSION-FLOOR unified the floor at 1.9, so
those phrasings now all state 1.9 and the "exception set" is simply the floor.
The three-phrasings lesson is the part that stays load-bearing — it is why
`verify.sh` §11 needle-checks exact fragments instead of grepping a pattern.)

**Every quoted cell is machine-checked, by a script that reads this table.**
`scripts/claims-check.mjs` parses the rows below, extracts each code span from
the **Now reads** column, and asserts it appears within the named line range.
Before phase 2 that column held the *pre-correction* text and the script proved
the pointers had not rotted; it now holds the *applied* text, so the same script
proves the corrections are still in place. The output line is deliberately
unchanged — run it with `node scripts/claims-check.mjs` and demand this exact
line, not just a zero exit:
...
A source-side check cannot see this class by construction — the cell matches the
file it quotes and still publishes wrongly. So the check of record is the built
page:
...
| --- | --- | --- |
| `≥ *1\.8` | 3 | A regex pattern quoted while explaining why bold markers defeat a naive sweep — once in the sweep-technique paragraph (`:430`), once in this inventory row, and once in the left-alone entry for `docs/validation-matrix.md:58`. The backslash escapes the dot *in the regex*, and the reader is meant to see it. |
...
on the old numbers — corrections that reflow a paragraph (L1, L16, L18, L26,
L30, L31) shift everything under them.
...
Running it also surfaced two rows (L4, L15) whose backslash-escaped code spans do
not render as intended in CommonMark, and confirmed L5's quote spans lines
134–135 rather than sitting on 135 alone.
...
| L5 | `pages/S14-security-scanners/index.md` | 134-135 | speaker note spanning two lines — ``Trivy config is the successor to tfsec — teach`` / `` `trivy config`, not the superseded binary. `` | line 135 read ``not a dead binary.`` | F2 |
| L6 | `pages/S14-security-scanners/index.md` | 169-170 | ``they are the workshop's "don't teach the superseded or archived`` / ``tool" pair.`` | line 169 read ``"don't teach the dead``, wrapping into ``tool" pair.`` on 170. | F2 |
| L7 | `pages/S14-security-scanners/index.md` | 418 | `- **tfsec → Trivy** (superseded); **Terrascan → archived** — teach neither.` | was `- **tfsec → Trivy**; **Terrascan → archived** — do not teach ghosts.` | F2 |
| L8 | `pages/S14-security-scanners/index.md` | 195 | `- **HashiCorp products only** — HCP Terraform / Terraform Enterprise, Vault, Nomad` | was `- **TFC / HCP Terraform only**`. | F11 |
| L9 | `pages/S14-security-scanners/index.md` | 197 | `- Fine inside those products — not a portable default` | was `- Fine inside that product — not a portable default`. | F11 |
| L10 | `pages/S14-security-scanners/index.md` | 204-205 | speaker note — `Sentinel is real and useful, but it` / `is tied to HashiCorp's own products; do not present it as the OpenTofu-first` | line 205 read `is tied to HashiCorp's cloud product; …`. | F11 |
| L11 | `pages/S14-security-scanners/index.md` | 421 | `- Sentinel stays inside HashiCorp's products; portable policy prefers OPA.` | was `- Sentinel stays TFC-only; portable policy prefers OPA.` — the recap slide, and the last place the CONTRADICTED claim survived. | F11 |
| L12 | `pages/S14-security-scanners/index.md` | 242 | `version = "~> 6.0"` (indented 6 spaces) | was `version = "~> 5.0"`. **All three copies of this pin moved together** — the slide, the tracked fixture, and the drift-armed lab block; see the L12 trap note below. | E3, E6, E7, F12 |
| L13 | `pages/S28-ecosystem-tooling/index.md` | 86 | `Terraform, Terragrunt, Terramate, Atmos**.` (indented 2 spaces) | was `Terraform, Terragrunt, Atmos**.` — tenv also manages Terramate. | G3 |
| L14 | `pages/S28-ecosystem-tooling/index.md` | 96 | `tfenv and tofuenv, is one binary that manages OpenTofu, Terraform, Terragrunt, Terramate,` | the same line without `Terramate,`; line 97 already read `and Atmos`. | G3 |
| L15 | `pages/S28-ecosystem-tooling/index.md` | 88 | ``- **Not** part of `task setup` here — any `tofu ≥ 1.9` (the workshop floor) runs every lab. Adopt`` | phase 2 wrote ``runs most labs (Labs 06 and 10 need ≥ 1.9)`` over the original false sufficiency claim (``any `tofu ≥ 1.8` runs the labs``); US-D-VERSION-FLOOR (2026-08-31) then unified the enforced floor at 1.9, so the "most labs" carve-out no longer exists and the cell was re-derived to the new text. | E2 |
| L16 | `pages/S28-ecosystem-tooling/index.md` | 99-100 | speaker note — `workshop deliberately does not require tenv — any tofu one-point-nine or newer,` / `the workshop floor, runs every lab — so the takeaway is a` | phase 2 replaced the duplicated `works —` opener with the most-labs carve-out; US-D-VERSION-FLOOR (2026-08-31) unified the floor at 1.9 and the note now speaks the floor, not the exception list — cell re-derived to the new text. | E2 |
...
| L26 | `labs/day-3/28-ecosystem-tooling.solution.md` | 38-39 | ``on purpose: `tofu ≥ 1.9` — the`` / ``workshop floor — runs every lab.`` | read `` on purpose: any `tofu ≥ 1.8` `` / ``runs the labs.`` — the same false sufficiency claim as L15, in lab prose rather than the deck; phase 2 substituted the most-labs carve-out, and US-D-VERSION-FLOOR (2026-08-31) re-derived the cell after unifying the floor at 1.9. | E2 |
| L27 | `labs/day-3/28-ecosystem-tooling.md` | 90 | ``Terragrunt, Terramate, and Atmos**.`` | the line began ``Terragrunt, and Atmos**.`` | G3 |
| L28 | `labs/day-3/28-ecosystem-tooling.solution.md` | 29-30 | ``one binary for OpenTofu, Terraform, Terragrunt, Terramate,`` / ``and Atmos — is how a laptop mirrors that pin per project:`` | the first line ended ``Terragrunt, and`` and the second began ``Atmos — …``; inserting Terramate moved the break one word. | G3 |
| L29 | `docs/associate-alignment.md` | 69 | ``HashiCorp products only (HCP Terraform / Terraform Enterprise, Vault, Nomad).`` | the **Sentinel** policy-authoring row's second cell read ``TFC/HCP-only.`` — the same CONTRADICTED claim as L8/L11, mirrored into the certification-alignment doc. | F11 |
| L30 | `labs/day-1/01-iac-fork.md` | 338-339 | ``So OpenTofu stays **MPL 2.0** (truly open source, Linux-Foundation-governed,`` / ``CNCF Sandbox since 2025-04-23) and keeps HCL compatibility`` | line 338 read ``…Linux-Foundation-governed) and``. Present-tense governance, so it took the same update as L19; the paragraph was reflowed onto four lines. | B6 |
...
source file` and `verify FAILED — 1 failure(s) across 138 check(s)`. Phase 2
applied all three in a single change and `verify.sh` stayed green.
...
| `pages/S01-iac/index.md:720` (B6) | Compressed one-line recap of the licence contrast. One insertion at `:352` is enough for the deck to state current governance; padding the recap costs clarity for no added truth. |
...
| `pages/S01-iac/index.md:627-635, 669-671` (I4) | I4 is PARTIAL because the fetch returned Ansible's agentless property as a paraphrase rather than the literal adjective. The deck's substance matches the source; there is nothing to correct, only a weaker-than-hoped citation to record honestly. |
...
| `pages/S19-testing-cicd/index.md:94` ("OpenTofu ≥ 1.9 preflight") | Was left alone at "≥ 1.8" because it accurately reported the gate's then-threshold. **Superseded 2026-08-31 (US-D-VERSION-FLOOR):** the preflight now enforces `TOFU_FLOOR` (1.9) and the slide moved with it, in the same change — exactly the coupling the out-of-scope note below demanded. |
...
| `versions.env:17, 27, 32, 36` (D1–D4) and `labs/day-2/14-security-scanners.md:8` (D5) | Pins, not prose. Deliberately untouched by this docs-only lane; carried in the out-of-scope section below. |
| `labs/day-1/00-setup/versions.tf:2` and ~20 peers (E1) | Left alone by this docs-only pass: the `>= 1.8` floor was *correct* for the content it guards; only the sufficiency claim was E2's business (L15–L18). **Superseded 2026-08-31 (US-D-VERSION-FLOOR):** the lab-workdir pins (and their drift-checked fences) now state the unified workshop floor `>= 1.9`. Deliberately NOT swept: `labs/day-1/10-differentiators/` (its two pins teach feature floors), `examples/**` and `modules/**` (artifact-scoped feature floors with reason comments), and `labs/day-2/19-*` (frozen under an in-flight lane at the time — its `>= 1.8` is below the floor, so it is lax, not contradictory, and `verify.sh` §11's ceiling scan still covers it). |
...
| `pages/S11-taco-landscape/index.md:65` and `:95` (generic policy-engine lists: "A policy engine (OPA/Rego, Sentinel, …", "or Sentinel or built-in engine …") | Generic enumerations of the policy-engine category. Neither says where Sentinel runs, so neither can be wrong in the way F11 is. Recorded because a phase-2 `Sentinel` grep returns these two alongside `:131` and the three corrected sites, and an unexplained hit invites a well-meaning wrong edit. |
...
> **Resolution note, 2026-08-31 (US-D-VERSION-FLOOR).** The three floor
> bullets below all LANDED: `MIN_TOFU` and the `verify.sh` preflight (now a
> single `TOFU_FLOOR` definition) enforce 1.9, the selftest literal moved with
> them, Lab 06 gained its `required_version = ">= 1.9"` guard (in `main.tf`,
> with the cross-variable rationale as a comment), and the lab-workdir pins
> plus `docs/validation-matrix.md:50/:58/:67` and `pages/S19:94` were
> corrected in the same change — the exact coupling the third bullet
> prescribed. A new `verify.sh` §11 skew gate (three `verify-selftest.sh`
> cases) needle-checks every consumer that restates the floor and scans
> labs/modules/examples for any artifact demanding more than it, so the
> cluster cannot silently re-diverge. The bullets are kept verbatim as the
> record of WHY; their line pointers and present-tense claims describe the
> pre-unification tree.
...
- **`labs/day-1/06-variables/` has no `required_version`.** Lab 06 needs OpenTofu
  1.9 (E2) but declares no engine constraint, so a learner on 1.8 gets
  `Invalid reference in variable validation` instead of a version error. Adding
  `required_version = ">= 1.9.0"` to `labs/day-1/06-variables/main.tf` would
  make it fail like Lab 10 does. This is a fixture change, not prose, and it
  carries **no** L row, so **phase 2 did not apply it either** — it stays open,
  and it is still the single highest-value follow-up here, because it converts
  the workshop's most obscure failure into an obvious one on its most-reached
  path.
- **Raising the advertised floor to 1.9 (L20–L26) must move the enforcers too —
  ...
  `pass "tofu v1.10.3 (>= 1.8)"` — which is outside a prose lane's blast
  radius, and doing it blind is how a docs change reds a gate it never read.
  **State the consequence precisely rather than calling it done:** the docs are
  now correct and the enforcers are **known-lax** — they accept a 1.8 that Labs
  06 and 10 will reject. Nothing else in this file becomes false, because
  `docs/validation-matrix.md:50` and `:58` and `pages/S19:94` report the
...
- `versions.env:21` (pre-bump) pinned `GO_VERSION=1.23.6`, a Go series that no longer
...
## M. S01 migration beat (US-D-MIGRATE-BEAT, added 2026-08-30)

The Terraform→OpenTofu migration beat added to S01 introduces claims that the
2026-08-25 pass never examined, so they are verified here at authoring time
rather than folded into that pass's census (the "57 verified" figure above
describes the original sweep only; these rows are additive). Letters J–L were
already taken by this document's own structure when the beat landed, hence M.

Capture environment for the demo rows: Terraform v1.15.8 and OpenTofu v1.12.5
on darwin_arm64, 2026-08-30, in a throwaway workdir holding Lab 01's
`random_pet` + `local_file` pair (providers `hashicorp/random` v3.9.0,
`hashicorp/local` v2.9.0). Sequence executed: `terraform init` → `terraform
apply -auto-approve` → `terraform plan` ("No changes") → `tofu init` → `tofu
plan` → `tofu apply -auto-approve`. The capture was re-run in full during the
review fix round (same versions, same sequence, same day); the ids shown are
from that final run. The slide fence marks **every** elision with `...` — the
whole lines it skips, and one in-line cut: the `local_file.manifest` id, a
40-hex SHA-1, is shown as its first 8 characters. Everything else the fence
shows is verbatim from that run.

| # | Claim | Where | Verdict | Evidence (primary source, checked 2026-08-30) |
| --- | --- | --- | --- | --- |
| M1 | "most Terraform code will work without modification" is the project's own, deliberately scoped compatibility claim | `pages/S01-iac/index.md:383-384, 390, 552-554, 580-582` | VERIFIED | `website/docs/intro/migration/index.mdx` at tag `v1.12.6`: "OpenTofu aims to maintain compatibility with Terraform configurations. While most Terraform code will work without modification, we recommend following our migration guide to ensure a smooth transition." Quoted with its own hedge intact — the deck presents it as scoped, not absolute. |
| M2 | Official migration path: back up state and code first, then `tofu init` → `tofu plan`, expecting "No changes" or the same plan Terraform showed; an unexpected diff means do not apply; rollback is documented ("safe and reversible") | `pages/S01-iac/index.md:513-518, 533-537, 722` | VERIFIED | `website/docs/intro/migration/migration-guide.mdx` at tag `v1.12.6`: Step 1 "Back up your infrastructure"; Step 4 "**Expected result**: You should see \"No changes\" or the same plan output you would see with Terraform." followed by "If you see unexpected changes: 1. Do not apply the changes"; "Rolling back to Terraform … you can safely roll back"; intro: "It is designed to be safe and reversible". |
| M3 | The first `tofu apply` "updates the state file format if needed" and stamps OpenTofu's version into the state | `pages/S01-iac/index.md:557-560, 583-587` | VERIFIED | Doc half: same `migration-guide.mdx` @ `v1.12.6`, Step 5: "Even if there are no infrastructure changes, this ensures OpenTofu updates the state file format if needed." Observed half (capture env above): before `tofu apply`, `terraform.tfstate` read `"version": 4, "terraform_version": "1.15.8"`; after, `"version": 4, "terraform_version": "1.12.5"` — format version unchanged, engine stamp rewritten. The slide scopes the "same format" half to the demo ("in the demo … on both sides") rather than asserting it universally. |
| M4 | `tofu init` rewrites `.terraform.lock.hcl` from `registry.terraform.io` to `registry.opentofu.org`; "version selections were preserved, but the hashes were not" | `pages/S01-iac/index.md:507-510, 526-530` | VERIFIED | Captured verbatim from the demo's `tofu init` (OpenTofu v1.12.5): "Warning: Dependency lock file entries automatically updated / OpenTofu automatically rewrote some entries in your dependency lock file: / - registry.terraform.io/hashicorp/local => registry.opentofu.org/hashicorp/local / - registry.terraform.io/hashicorp/random => registry.opentofu.org/hashicorp/random / The version selections were preserved, but the hashes were not because the OpenTofu project's provider releases are not byte-for-byte identical." |
| M5 | The `tofu plan` handshake against Terraform-written state: "No changes. Your infrastructure matches the configuration." | `pages/S01-iac/index.md:497-500, 530-533` | VERIFIED | Captured verbatim from the demo's `tofu plan` (state written by Terraform v1.15.8, read by OpenTofu v1.12.5) — two resources, two refresh lines: "random_pet.env: Refreshing state... [id=clever-chipmunk]" and "local_file.manifest: Refreshing state... [id=b643126fa1e0a8fcedc859cf569cef6ee33b12cb]" (the fence shortens this id to `b643126f...`), then "No changes. Your infrastructure matches the configuration." |
| M6 | 1.9-era docs shipped per-Terraform-version guides saying migrate like-to-like then upgrade, and wait if Terraform is newer than the newest guide; today's guide is version-generic | `pages/S01-iac/index.md:571-574, 591-596` | VERIFIED (dated) | `website/docs/intro/migration/terraform-1.9.mdx` at tag `v1.9.0`: "please migrate to OpenTofu 1.9.0 first, then upgrade your OpenTofu installation to the latest version" and "If you are on a higher Terraform version, please wait until an appropriate migration guide becomes available." The per-version pages (`terraform-1.5-or-lower` … `terraform-1.9`) exist at tag `v1.9.0` and are gone at tag `v1.12.6`, where only the generic `migration-guide.mdx` remains — both halves checked against the tagged trees. The deck dates the claim ("1.9 era") precisely because it is no longer current doctrine. |
| M7 | tenv installs and pins OpenTofu **and** Terraform side by side | `pages/S01-iac/index.md:562-565, 588-591` | VERIFIED | Same evidence base as G1/G3/G4 (tenv `README.md`: "OpenTofu, Terraform, Terragrunt, Terramate and Atmos version manager, written in Go"); the S01 card claims only the OpenTofu+Terraform subset relevant to a migration and points at S28 for the full list. |

One deliberate omission, recorded so it is not mistaken for an oversight: the
demo ran a **newer** Terraform (1.15.8) into an **older** OpenTofu (1.12.5)
cleanly, and the slide says exactly that and no more. No claim is made that
this holds for arbitrary version pairs or resource types — the 1.9-era "wait"
guidance (M6) is presented as the historical counterweight, and "your plan
decides" is the operative instruction.
