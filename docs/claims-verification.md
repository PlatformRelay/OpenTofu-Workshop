...
| A9 | The 1.7→1.12 span is a genuine divergence window — "several **with no Terraform equivalent**"; provider `for_each` is "**OpenTofu-only** since 1.9" | `pages/S10-.../index.md:39-40, 147, 291, 303`; `pages/S01-iac/index.md:380-384, 421-422, 715` | VERIFIED | Two halves, checked separately. (a) Every feature listed in A3–A8 appears in OpenTofu's own changelog at the version claimed. (b) The *exclusivity* half was checked against **HashiCorp's** docs, not inferred: provider `for_each` — `https://developer.hashicorp.com/terraform/language/providers/configuration` documents only aliasing ("include multiple `provider` blocks with the same provider name, then add the `alias` argument…") and never mentions `for_each` on a provider block; `-exclude` — `https://developer.hashicorp.com/terraform/cli/commands/plan` documents `-target=ADDRESS` as its only resource-targeting option, with no `-exclude`; client-side state encryption — `https://developer.hashicorp.com/terraform/language/state/sensitive-data` states "You can encrypt your state at rest, but the encryption method depends on your specific backend", i.e. delegated to S3/GCS/HCP rather than a native `encryption` block with key providers. All three OpenTofu features named as differentiators are genuinely absent from Terraform. |
...
| `pages/S01-iac/index.md:714` (B6) | Compressed one-line recap of the licence contrast. One insertion at `:352` is enough for the deck to state current governance; padding the recap costs clarity for no added truth. |
...
| `pages/S01-iac/index.md:621-629, 663-665` (I4) | I4 is PARTIAL because the fetch returned Ansible's agentless property as a paraphrase rather than the literal adjective. The deck's substance matches the source; there is nothing to correct, only a weaker-than-hoped citation to record honestly. |
...
plan` → `tofu apply -auto-approve`. The slide fence elides lines with `...`;
every line it does show is verbatim from that run.

| # | Claim | Where | Verdict | Evidence (primary source, checked 2026-08-30) |
| --- | --- | --- | --- | --- |
| M1 | "most Terraform code will work without modification" is the project's own, deliberately scoped compatibility claim | `pages/S01-iac/index.md:390, 546-548, 574-576` | VERIFIED | `website/docs/intro/migration/index.mdx` at tag `v1.12.6`: "OpenTofu aims to maintain compatibility with Terraform configurations. While most Terraform code will work without modification, we recommend following our migration guide to ensure a smooth transition." Quoted with its own hedge intact — the deck presents it as scoped, not absolute. |
| M2 | Official migration path: back up state and code first, then `tofu init` → `tofu plan`, expecting "No changes" or the same plan Terraform showed; an unexpected diff means do not apply; rollback is documented ("safe and reversible") | `pages/S01-iac/index.md:509-514, 528-532, 716` | VERIFIED | `website/docs/intro/migration/migration-guide.mdx` at tag `v1.12.6`: Step 1 "Back up your infrastructure"; Step 4 "**Expected result**: You should see \"No changes\" or the same plan output you would see with Terraform." followed by "If you see unexpected changes: 1. Do not apply the changes"; "Rolling back to Terraform … you can safely roll back"; intro: "It is designed to be safe and reversible". |
| M3 | The first `tofu apply` "updates the state file format if needed" and stamps OpenTofu's version into the state | `pages/S01-iac/index.md:551-554, 577-581` | VERIFIED | Doc half: same `migration-guide.mdx` @ `v1.12.6`, Step 5: "Even if there are no infrastructure changes, this ensures OpenTofu updates the state file format if needed." Observed half (capture env above): before `tofu apply`, `terraform.tfstate` read `"version": 4, "terraform_version": "1.15.8"`; after, `"version": 4, "terraform_version": "1.12.5"` — format version unchanged, engine stamp rewritten. The slide scopes the "same format" half to the demo ("in the demo … on both sides") rather than asserting it universally. |
| M4 | `tofu init` rewrites `.terraform.lock.hcl` from `registry.terraform.io` to `registry.opentofu.org`; "version selections were preserved, but the hashes were not" | `pages/S01-iac/index.md:503-506, 522-526` | VERIFIED | Captured verbatim from the demo's `tofu init` (OpenTofu v1.12.5): "Warning: Dependency lock file entries automatically updated / OpenTofu automatically rewrote some entries in your dependency lock file: / - registry.terraform.io/hashicorp/local => registry.opentofu.org/hashicorp/local / - registry.terraform.io/hashicorp/random => registry.opentofu.org/hashicorp/random / The version selections were preserved, but the hashes were not because the OpenTofu project's provider releases are not byte-for-byte identical." |
| M5 | The `tofu plan` handshake against Terraform-written state: "No changes. Your infrastructure matches the configuration." | `pages/S01-iac/index.md:494-497, 526-528` | VERIFIED | Captured verbatim from the demo's `tofu plan` (state written by Terraform v1.15.8, read by OpenTofu v1.12.5): "random_pet.env: Refreshing state... [id=up-treefrog]" then "No changes. Your infrastructure matches the configuration." |
| M6 | 1.9-era docs shipped per-Terraform-version guides saying migrate like-to-like then upgrade, and wait if Terraform is newer than the newest guide; today's guide is version-generic | `pages/S01-iac/index.md:565-568, 585-590` | VERIFIED (dated) | `website/docs/intro/migration/terraform-1.9.mdx` at tag `v1.9.0`: "please migrate to OpenTofu 1.9.0 first, then upgrade your OpenTofu installation to the latest version" and "If you are on a higher Terraform version, please wait until an appropriate migration guide becomes available." The per-version pages (`terraform-1.5-or-lower` … `terraform-1.9`) exist at tag `v1.9.0` and are gone at tag `v1.12.6`, where only the generic `migration-guide.mdx` remains — both halves checked against the tagged trees. The deck dates the claim ("1.9 era") precisely because it is no longer current doctrine. |
| M7 | tenv installs and pins OpenTofu **and** Terraform side by side | `pages/S01-iac/index.md:556-559, 582-585` | VERIFIED | Same evidence base as G1/G3/G4 (tenv `README.md`: "OpenTofu, Terraform, Terragrunt, Terramate and Atmos version manager, written in Go"); the S01 card claims only the OpenTofu+Terraform subset relevant to a migration and points at S28 for the full list. |
