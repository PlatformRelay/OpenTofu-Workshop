# Lab 24 — Change detection & filtering — solutions
...
Work from the tracked workdir `labs/day-3/24-change-detection/` unless a step says otherwise.
...
Re-enter `labs/day-3/24-change-detection/` and replay from the failing step. To fully reset generated state, run `tofu destroy -auto-approve` when the lab created resources, then `tofu init -upgrade` and retry `tofu plan`.
