# Lab 17 — Mocking providers (`mock_provider`)

| | |
| --- | --- |
| **Section** | S17 — Mocking providers |
...
credentials**, then prove the green path while LocalStack / Docker is **down**.
Deliberately break a wrong `override_resource` default, read the failure, and
...
- Docker may be installed, but this lab must succeed with the workshop
  LocalStack container **stopped**

## Files used

- [`labs/day-2/17-mocking/main.tf`](./17-mocking/main.tf) — deterministic S3 root
- [`labs/day-2/17-mocking/tests/unit.tftest.hcl`](./17-mocking/tests/unit.tftest.hcl) —
  mocked plan suite (`mock_provider` + `override_resource`)
