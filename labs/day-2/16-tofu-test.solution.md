# Lab 16 — Native testing with `tofu test` — solutions
...
Work from the tracked workdir `labs/day-2/16-tofu-test/` unless a step says otherwise.
...
Re-enter `labs/day-2/16-tofu-test/` and replay from the failing step. To fully reset generated state, run `tofu destroy -auto-approve` when the lab created resources, then `tofu init -upgrade` and retry `tofu plan`.
