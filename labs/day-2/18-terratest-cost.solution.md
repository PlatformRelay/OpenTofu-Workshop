# Lab 18 — Terratest vs LocalStack (+ optional Infracost) — solutions
...
Work from the tracked workdir `labs/day-2/18-terratest-cost/` unless a step says otherwise.
...
Re-enter `labs/day-2/18-terratest-cost/` and replay from the failing step. To fully reset generated state, run `tofu destroy -auto-approve` when the lab created resources, then `tofu init -upgrade` and retry `tofu plan`.

## Stretch solution

### Commands / manifest

— Infracost breakdown (optional · free API key)
