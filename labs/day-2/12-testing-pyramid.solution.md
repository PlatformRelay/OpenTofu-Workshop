# Lab 12 — Classify the evidence — solutions
...
Work from the tracked workdir `labs/day-2/12-testing-pyramid/` unless a step says otherwise.
...
Re-enter `labs/day-2/12-testing-pyramid/` and replay from the failing step. To fully reset generated state, run `tofu destroy -auto-approve` when the lab created resources, then `tofu init -upgrade` and retry `tofu plan`.
