...
### Continuity — stage 3 of the `service-manifest` project

**Carried forward from stage 2** (`labs/day-1/02-hcl-blocks/`): the spine
`local_file.manifest` and `output "manifest_path"`, plus the auxiliary
`random_pet.env`.

**Deliberately retired here — four blocks stage 2 used to demonstrate the block
taxonomy, whose teaching job is done:** `variable "owner"`, the `locals` block,
`data.local_file.motd` and `module "greeting"`. S03 teaches how to *read a plan*,
and a plan with fewer moving parts is easier to read. None of them was a spine
address, so nothing the project needs is lost — the project's real inputs,
`variable "service"` and `variable "environment"`, arrive at stage 4
(`labs/day-1/06-variables/`).

**Introduced here, and auxiliary:** `local_file.summary` — it exists only to give
the dependency graph a second edge. It retires at stage 4, which says so.
...
- Change `random_pet`'s `length` from `2` to `3`, run `tofu plan`, and read how
  ...
  -/+ resource "local_file" "manifest" {
        ...
        ~ content_sha256       = "c29e0102...fd0e39e9" -> (known after apply)
        ~ id                   = "0ac42dc1...6d4865f0" -> (known after apply)
          # (3 unchanged attributes hidden)
      }

    # random_pet.env must be replaced
  -/+ resource "random_pet" "env" {
  ...
  Changing `length` **forces replacement** of `random_pet.env` (marked
