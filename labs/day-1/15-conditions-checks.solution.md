# Lab 15 — Preconditions, postconditions & check blocks (S15) — solutions
...
Work from the tracked workdir `labs/day-1/15-conditions-checks/` unless a step says otherwise.
...
Re-enter `labs/day-1/15-conditions-checks/` and replay from the failing step once the environment is clean. For provider or module download errors, run `tofu init -upgrade` in the workdir and retry `tofu plan`.
